...
[br,bc] = size(Img);
Img(br+1:br+10,:) = 1e-13;	
Img(:,bc+1:bc+10) = 1e-13;

lim_az = etaTotal+10;
lim_rg = length(FastTime)+10;

p = FP08_FastPeakFind(Img);
imagesc(Img(1:br,1:bc)); %returns normal image
hold on
...
%% X axis
figure
hx = histogram(x_axs,round(450/11));
title('hx');
...
%% Y axis
figure
hy = histogram(y_axs,round(2*800/11));
title('hy');
hbiny(:,1) = round(hy.BinEdges);
hbiny(1,1) = 1;%can not zero
hbiny(:,2) = [hy.BinCounts 0];
...
%sort again based on bins
hbin4y = sortrows(hbin3y,1,'ascend');

%create cell border

%for the first row
hbin4y(1,3) = hbin4y(1,1) - round(minus(hbin4y(1,1),0)/brds);
hbin4y(1,4) = hbin4y(1,1) + round(minus(hbin4y(2,1),hbin4y(1,1))/brds);

%for the last row
hbin4y(length(hbin4x),3) =  hbin4y(length(hbin4y),1) -  round(minus(hbin4y(length(hbin4y),1),hbin4y(length(hbin4y)-1,1))/brds);
hbin4y(length(hbin4y),4) =  hbin4y(length(hbin4y),1) +  round(minus(lim_az,hbin4y(length(hbin4y),1))/brds);
%hbin4y(length(hbin4y),4) = etaTotal;
...
for gy = 1:11
	for gx = 1:11
	    ...
	    %plot(val_cell(ix,1),val_cell(ix,2) ,'r+');%plot plus sign at target points
...
text(grp_rg-30, grp_az-20,'GRP', 'FontSize', 13, 'Color', 'k','Interpreter','latex');
text(grp_rg+5, grp_az-180,'$LGRP_{a}$', 'FontSize', 13, 'Color', 'b','Interpreter','latex');
text(grp_rg+78, grp_az-20,'$LGRP_{r}$', 'FontSize', 13, 'Color', 'b','Interpreter','latex');
text(c_rg+10, a_az+20,'$L_{1}$', 'FontSize', 13, 'Color', "#D95319", 'Interpreter','latex');
text(c_rg+10, b_az-20,'$L_{2}$', 'FontSize', 13, 'Color', "#D95319", 'Interpreter','latex');
text(c_rg+8,  grp_az-180,'$L_{3}$', 'FontSize', 13, 'Color', '#006400','Interpreter','latex');
text(d_rg-20,  grp_az-180,'$L_{4}$', 'FontSize', 13, 'Color', '#006400','Interpreter','latex');
text(a_rg-20, a_az+20,'$P_{1}$', 'FontSize', 13, 'Color', 'k', 'Interpreter','latex');
text(b_rg-20, b_az-20,'$P_{2}$',  'FontSize', 13, 'Color', 'k', 'Interpreter','latex');
text(c_rg+10, c_az+20,'$P_{3}$', 'FontSize', 13, 'Color', 'k','Interpreter','latex');
text(d_rg-20, d_az-20,'$P_{4}$', 'FontSize', 13, 'Color', 'k','Interpreter','latex');
...
subtt = sprintf('$GRP_{r}$ = %.1f dB',MagArr(2,16));
title(subtt, 'Units', 'normalized', 'Position', [0.1, 0.1, 0],'horizontalAlignment', 'left','interpreter','latex','FontSize',10,'BackgroundColor','w','EdgeColor','k');
...
FP07_LinePlot(1,idx_mg,mag,MagArr(4,8),MagArr(4,11),subtt,0,0)
subtt = sprintf('$L_3$');
title(subtt, 'Units', 'normalized', 'Position', [0.5, 0.1, 0],'horizontalAlignment', 'left','interpreter','latex','FontSize',10,'BackgroundColor','w','EdgeColor','k');
...
FP07_LinePlot(2,idx_mg,mag,MagArr(4,8),MagArr(4,11),subtt,0,0, MagArr(4,7), MagArr(4,9), MagArr(4,15), MagArr(4,17), MagArr(4,18))
...
FP07_LinePlot(2,idx_mg,mag,MagArr(5,8),MagArr(5,11),subtt,0,0, MagArr(5,7), MagArr(5,9), MagArr(5,15), MagArr(5,17), MagArr(5,18))
...
FP07_LinePlot(1,idx_mg,mag,MagArr(6,8),MagArr(6,11),subtt,2,0)
subtt = sprintf('$L_4$');
title(subtt, 'Units', 'normalized', 'Position', [0.5, 0.1, 0],'horizontalAlignment', 'left','interpreter','latex','FontSize',10,'BackgroundColor','w','EdgeColor','k');
...
FP07_LinePlot(2,idx_mg,mag,MagArr(6,8),MagArr(6,11),subtt,2,0, MagArr(6,7), MagArr(6,9), MagArr(6,15), MagArr(6,17), MagArr(6,18))
