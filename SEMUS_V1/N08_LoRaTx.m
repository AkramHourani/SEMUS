function [signal_mod] = N08_LoRaTx(message,Bandwidth,SF,Pt,Fs,df,varargin)
...
signal_mod = 10.^(Pt./20).*signal.*exp(-1i.*2.*pi.*df/Fs.*(0:length(signal)-1))' ; % frquency shift and convert to power
end
...
y       = reshape(exp(1i.*2.*pi.*Theta),numel(Theta),1) ;
end