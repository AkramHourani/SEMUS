...
I = I(:).'; Q = Q(:).';                                         % Serialize
t = 0:1/fs:(length(I)-1)/QPSK.fs;                               % Time base vector for the carrier modulated signal
...
QPSKTime = length(signal_IQ)/QPSK.fs;
...
QPSK{i}(:,:) = padarray(sqpsk,[(round(size(sqd,1)*scale(i))-size(sqpsk,1)), (size(sqd,2)-size(sqpsk,2))],0,'pre');
sQPSKi = padarray(QPSK{i}(:,:),[(size(sqd,1)-size(QPSK{i}(:,:),1)), (size(sqd,2)-size(QPSK{i}(:,:),2))],0,'post');
sQPSK = sQPSKi +sQPSK;
end
% imagesc(real(sQPSK))
N05_QPSKGain
