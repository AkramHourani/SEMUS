...
% speed= mean(sqrt(sum((diff(pos,[],2)).^2)) /Param.ts);                          % Platform speed = sqrt(Param.mu/(h+Re))
...
AWGN = 0.001*sqrt(P_N)* (randn(size(sqd)) + 1i * (randn(size(sqd)) ) );         % Generate the AWGN the same size as recieved signal 
