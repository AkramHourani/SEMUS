...
%% creating video
% tlat = (SatllaI(1,1) + Targetlat(1,1))/2;%-33.8263
% tlon = (SatllaI(1,2) + Targetlon(1,1))/2;%152.9229
% talt = SatllaI(1,3)*4;% 2.0539e+06
% 
% close all hidden
% uif = uifigure;
...
% cz = 8.9375e+05;
% 
% %campos(g,tlat,tlon,talt/2)
% campos(g,cx,cy,cz)
% campitch(g,-60)
% geoplot3(g,SatllaI(1,1),SatllaI(1,2),SatllaI(1,3),'mo');  % Satellite subline
% hold(g,'on')
% 
% Targetalt = zeros(11,11);
% 
% for i = 1 : 11
%     geoplot3(g,Targetlat(i,:),Targetlon(i,:),0,"mo");   % target points
%     hold(g,'on')
%     geoplot3(g,[Targetlat(i,:) SatllaI(1,1)],[Targetlon(i,:) SatllaI(1,2)],[Targetalt(i,:) SatllaI(1,3)],"y","LineWidth",2);
%     hold(g,'on')
% end
% hold(g,'off')
% 
% [az1,elev1,slantRangeSrc1] = geodetic2aer(Targetlat,Targetlon,0,SatllaI(1,1),SatllaI(1,2),SatllaI(1,3),E);

%%%%%end video



% 1.a Find azimuth, elevaiton, and slantrange for each point target
