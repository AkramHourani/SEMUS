...
So = fft1d2(sqd);                                                           % FFT for the time domain raw signal (FFT along each eta row)
...
So_ref = fft1d2(sqd_ref);                                                   % FFT for the time domain reference signal (FFT along each eta row)
...
RangeBin = RadPar.ts*c/2;                                                   % Ground range resolution
...
%%% Analytical Matched Filter %%%%
Haz = exp(1j*pi*R*4*RadPar.fo/c);                                           % Azimuth Matched Filter
