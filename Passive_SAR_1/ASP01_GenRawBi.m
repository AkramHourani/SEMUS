...
%1b. Finding the swath of the Passive SAR / Satellite of Interest
...
%2b. Finding the swath of the Active SAR / Interferer
...
% 3b. Distance from swath of Passive SAR (SoI) to the Passive SAR(SoI) --> Target -->Rx
[~,~,RSoI]  = geodetic2aer(latSwathMidSoI(MidEta),lonSwathMidSoI(MidEta),0,SatllaSoI(:,1),SatllaSoI(:,2),SatllaSoI(:,3),E);

% 3c. Distance from the Active SAR to the swath Passive SAR --> Tx to Target 
[~,~,RI]    = geodetic2aer(latSwathMidSoI(MidEta),lonSwathMidSoI(MidEta),0,SatllaI(:,1),SatllaI(:,2),SatllaI(:,3),E);

% 3d. Find total distance Tx --> Target --> Rx
Ro          = min(RSoI+RI);% later will be used for calculating bistatic delay
...
% 7b. Plotting the chirp
figure(4)
figure_4

%% 8. (Optional) you can select the Testing value for testing the script
% 0 = optical processing    |   1 = GRP
% 2 = few targets testing   |   3 = unity reflection
% This simulation : TargetLat --> 11 x 11   |  TargetLon --> 11 x 11

Testing = 3 ; 

% 8a.this is for single targets testing
if Testing == 1 
    Targetlat   = GRP(1);
    Targetlon   = GRP(2);
    a = 1;
...
% 8b. This is for five targets testing
NTesting = 5;
if Testing == 2 % 
...
% 8c. This will force the reflectivity to unity
if Testing == 3 
    a           = 1;
    FileName    = 'Points.mat';
end
